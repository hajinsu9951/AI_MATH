...
| Ⅲ. 이미지 데이터 처리 | 14~20 | imgop · rgb · transpose · matmul · imgcls · fclayer · cnn | 7/7 완료 |
| Ⅳ. 예측과 최적화 | 21~30 | prob(21·22) · trend(23·24) · optim(25·27·28) · loss2(26) · gdsheet(29) · axb(30) | 10/10 완료 |
| Ⅴ. 인공지능과 수학 탐구 | 31~34 | inquiry · decision · datalab · project | 4/4 완료 |
| Ⅵ. 생성형 AI와 수학 (선택 심화 · 평가 제외) | 35~37 | genvec · genimg · genethics | 3/3 완료 |

**37차시 전부 가동** · 뷰 33개 · 콘솔 오류 0 · 375/1280px 가로 넘침 0

## 화면 조작

- **상단 내비** `AI 수학 | 홈 | 차시 목록 ▾ | [◀] N차시 · 제목 [▶]` — 양옆 버튼으로 이전·다음 차시 즉시 이동(대상 차시 제목 툴팁, 첫·마지막 비활성, 홈에서 자동 숨김)
- **하단 이동 바** ← 이전 / 37차시 중 N / 다음 → , 화면 가장자리 원형 화살표, **키보드 ←/→**(입력 중에는 비활성)
- **학습지** [🖨 학습지 인쇄] + [💾 다른 이름으로 저장] — 파일명을 정해 `.doc`로 저장하면 구글 드라이브 업로드 시 구글 문서로 변환됩니다(업로드 안내·드라이브 링크 자동 표시)
